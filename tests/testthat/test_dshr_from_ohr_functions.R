...
test_that("invalid inputs", {
  ...
  ### Hazard parameter estimates should be passed to calc_dshr_from_ohr if
  ### continuous-time hazard functional forms are selected
  expect_error(calc_dshr_from_ohr(n_age_init = n_age_init, 
                                  trial_time = trial_time, 
                                  hazard = "exponential"), 
               "`mu0` and `alpha` should not be NULL when hazard is either exponential, linear, or geometric")
  expect_error(calc_dshr_from_ohr(n_age_init = n_age_init, 
                                  trial_time = trial_time, 
                                  hazard = "linear"), 
               "`mu0` and `alpha` should not be NULL when hazard is either exponential, linear, or geometric")
